...
static void sdhci_finish_data(struct sdhci_host *host)
{
	...
	if (host->flags & SDHCI_REQ_USE_DMA) {
		...
		if (host->flags & SDHCI_USE_PLATDMA)
			/* we may have to abandom an ongoing platform DMA */
	...
	while (sdhci_readl(host, SDHCI_PRESENT_STATE) & mask) {
		if (timeout == 0) {
			printk(KERN_ERR "%s: Controller never released "
			       "inhibit bit(s) 0x%x so cmd %d abandoned.\n",
			       mmc_hostname(host->mmc), mask, cmd->opcode);
			sdhci_dumpregs(host);
			cmd->error = -EIO;
			tasklet_schedule(&host->finish_tasklet);
			return;
...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (!host->cmd) {
		if (!(host->quirks & SDHCI_QUIRK_EXTRA_INTS)) {
		printk(KERN_ERR "%s: Got command interrupt 0x%08x even "
					"though no command operation was in "
					"progress.\n",
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	if (host->flags &
	    (SDHCI_USE_SDMA | SDHCI_USE_ADMA | SDHCI_USE_PLATDMA)) {
		if (host->ops->enable_dma) {
